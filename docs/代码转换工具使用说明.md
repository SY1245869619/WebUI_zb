...
1. **选择模块** - 选择要保存的模块（动态获取所有启用的模块）
2. **输入测试名称** - 例如：`login`, `navigation`, `create_course`
3. **粘贴录制的代码** - 将Playwright录制的代码粘贴到输入框（输入框会根据内容自动调整大小）
4. **点击"转换"按钮** - 系统会自动转换代码
5. **查看转换结果** - 转换后的代码会在弹窗中显示，可以查看和确认
6. **点击"保存"按钮** - 确认转换结果无误后，点击保存按钮自动保存到对应的模块目录

**注意**：
- 代码输入框会根据粘贴的内容自动调整大小，方便查看完整代码
- "保存"按钮在转换完成前是禁用状态，只有转换成功后才会启用
- 转换结果会在弹窗中显示，方便查看和确认后再保存
...
- 文件命名格式：`test_{module_key}_{test_name}.py`
- 保存路径：`test_cases/{module_key}/test_{module_key}_{test_name}.py`

例如：
- **授课教学** (teaching) → `test_cases/teaching/test_teaching_{test_name}.py`
- **攻防演练** (exercise) → `test_cases/exercise/test_exercise_{test_name}.py`
- **考试测评** (exam) → `test_cases/exam/test_exam_{test_name}.py`
- **网络仿真** (simulate) → `test_cases/simulate/test_simulate_{test_name}.py`

**注意**：模块列表会根据 `config/module_config.yaml` 中的配置动态显示，所有启用的模块都会出现在选择列表中。
