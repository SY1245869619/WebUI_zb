...
├── utils/                   # 工具类
│   ├── recording_auto_login.py  # 录制工具自动登录
│   ├── report_parser.py          # 测试报告解析器
│   ├── custom_report_generator.py  # 自定义中文报告生成器
...
#### 3.1 pytest-html编码补丁（Windows系统）

**重要**：在Windows系统下，pytest-html生成的HTML报告可能会出现中文乱码。项目已包含自动补丁功能，会在首次运行时自动修复。

如果自动补丁失败，可以手动修改：
1. 找到pytest-html安装目录：`C:\Python310\lib\site-packages\pytest_html\html_report.py`
2. 找到第117行左右的 `html.meta(charset="utf-8")`
3. 改为：`html.meta(charset="GB2312")`
...
1. **配置登录信息** - 在"登录配置"面板配置登录URL、用户名和密码（重要！）
2. **选择应用模块** - 在左侧勾选要执行的应用模块
3. **配置通知** - 配置钉钉机器人和邮箱（可选）
4. **开始执行** - 点击"开始执行"按钮运行测试
5. **查看日志** - 实时查看执行日志

## 📝 编写测试用例

### ⭐ 使用Playwright Codegen录制用例（推荐给实习生）

**这是最简单的方式！就像录屏一样，但生成的是代码。**

#### 快速开始（3步搞定）

1. **启动录制工具（已自动登录）**：
   - **推荐方式**：通过Web界面，在"用例录制"面板输入桌面URL（如 `http://10.70.70.96/Shenyuan_9#/index`），点击"启动录制"
   - 或者使用自动登录脚本：`python utils/recording_auto_login.py http://10.70.70.96/Shenyuan_9#/index`
   - 浏览器会自动登录并关闭弹窗，您可以直接开始录制！✨
...
3. **保存代码**：
   - 停止录制后，复制生成的代码
   - 保存到对应文件夹：
     - 授课教学 → `test_cases/teaching/test_xxx.py`
     - 攻防演练 → `test_cases/exercise/test_xxx.py`
     - 考试测评 → `test_cases/exam/test_xxx.py`
...
| 应用模块 | 保存位置 | 文件命名示例 |
|---------|---------|------------|
| 授课教学 | `test_cases/teaching/` | `test_teaching_login.py` |
| 攻防演练 | `test_cases/exercise/` | `test_exercise_start.py` |
| 考试测评 | `test_cases/exam/` | `test_exam_create.py` |

**记住**：文件名以 `test_` 开头，使用下划线连接！
...
测试执行前会自动执行登录操作（如果启用了 `auto_login`）：
- 登录URL、用户名、密码可在Web控制界面配置
- 登录信息保存在 `config/settings.yaml` 中
- 支持自定义元素选择器（如果页面结构特殊）
...
try:
    ...
    db.execute_update("INSERT INTO users (name, email) VALUES (%s, %s)", ("张三", "zhangsan@example.com"))
finally:
    db.disconnect()
...
**SSH隧道支持**：
如果数据库需要通过SSH隧道访问，在 `config/settings.yaml` 中配置：
```yaml
database:
  host: 10.70.70.96
  port: 3306
...
支持从Excel表格导入测试用例，让非技术人员也能编写测试用例！
